...
func (m *Matcher) GetMatcherList() (matcherListInfo []*Matcher) {
	...
	for _, v := range matcherList {
		matcherListInfo = append(matcherListInfo, v.copy())
	}
	return
}

// SetPriority 设置当前 Matcher 优先级 默认优先级为5
...
// FirstPriority 设置当前 Matcher 优先级 - 5 需要更高的优先级可用SetPriority
func (m *Matcher) FirstPriority() *Matcher {
	return m.SetPriority(5)
}

// SecondPriority 设置当前 Matcher 优先级 - 6
func (m *Matcher) SecondPriority() *Matcher {
	return m.SetPriority(6)
}

// ThirdPriority 设置当前 Matcher 优先级 - 7
func (m *Matcher) ThirdPriority() *Matcher {
	return m.SetPriority(7)
}
