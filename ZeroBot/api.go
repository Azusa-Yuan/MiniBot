...
func formatMessage(msg interface{}) string {
	...
	default:
		...
		return utils.BytesToString(base64Reg.ReplaceAllFunc(s, func(b []byte) []byte {
			buf := bytes.NewBuffer([]byte(`"type":"image","data":{"file":"`))
			b = b[40:]
			b, err := base64.StdEncoding.DecodeString(utils.BytesToString(b))
			if err != nil {
...
func (ctx *Ctx) CallAction(action string, params Params) (APIResponse, error) {
	...
	if err == nil && resp.RetCode != 0 {
		...
		err = fmt.Errorf("调用 %s 时出现错误, %s", action, utils.BytesToString(respBytes))
		log.Error().Str("name", "api").Err(err).Msgf("%v", req)
	}
	return resp, err
}
