...
var dataPath = filepath.Join(path.GetDataPath(), "data")

func (dgtr datagetter) randImage(fileName ...string) message.MessageSegment {
	imgPath := filepath.Join(dataPath, fileName[rand.Intn(len(fileName))])
...
func (dgtr datagetter) randRecord(fileName ...string) message.MessageSegment {
	imgPath := filepath.Join(dataPath, fileName[rand.Intn(len(fileName))])
	data, err := os.ReadFile(imgPath)
	if err != nil {
		return message.Text("ERROR: ", err)
...
func init() { // 插件主体
	...
	var dgtr datagetter = datagetter{}
